@section('content')
<x-guest-layout>
    <form method="POST" action="{{ route('insert-riwayat')}}">
        @csrf
        <div class="insert text-center">
            <h1 style="color:black; font-size:30px;">Tambah Medical Record</h1>
        </div>
        
        <!-- Pasien ID -->
        <div class="mt-4">
            <x-input-label for="pasien_id" :value="__('Pasien ID')" />
            <x-text-input id="pasien_id" class="block mt-1 w-full" type="text" name="pasien_id"  required autofocus autocomplete="pasien_id" />
        ...
        <div class="mt-4">
            ...
            <x-text-input id="dokter_id" class="block mt-1 w-full" type="text" name="dokter_id" :value="auth()->user()->dokter->id" required autofocus autocomplete="dokter_id" readonly/>
